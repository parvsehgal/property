...
export default function InvestorsResourcesSection() {
  ...
  const calculateReturns = () => {
    ...
    return years.map((year) => {
      ...
      const capitalAppreciation = amount * Math.pow(1 + appreciation_rate, year) - amount
      return {
        year,
        rental: rentalIncome,
    ...
    <section className="py-16 bg-background border-t border-white/20">
      <div className="container max-w-6xl mx-auto">
        <div className="text-center mb-12">
          <h2 className="text-4xl md:text-5xl font-bold text-white mb-4">Investors Resources</h2>
          <p className="text-lg text-muted-foreground mb-4">Your one-stop resource for real estate investment.</p>
          <div className="w-24 h-1 bg-accent mx-auto"></div>
        </div>

        <div className="max-w-5xl mx-auto bg-white rounded-2xl p-8 border border-gray-200 shadow-xl">
          <h3 className="text-2xl font-bold text-gray-900 mb-8 text-center">ROI Investment Calculator</h3>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            {/* Left side - Controls */}
            <div className="space-y-8">
              <div>
                <label className="block text-gray-900 text-lg mb-4 font-semibold">Enter Investment Amount</label>
              ...
              <div>
                <div className="flex justify-between items-center mb-4">
                  <label className="text-gray-900 text-lg font-semibold">Net Rental Yield %</label>
                  <span className="text-accent font-bold text-lg">{rentalYield[0]}%</span>
                </div>
                <Slider
                  value={rentalYield}
              ...
              <div>
                <div className="flex justify-between items-center mb-4">
                  <label className="text-gray-900 text-lg font-semibold">Annual Appreciation %</label>
                  <span className="text-accent font-bold text-lg">{appreciation[0]}%</span>
                </div>
                <Slider
                  value={appreciation}
            ...
            <div>
              <div className="mb-6">
                ...
                <div className="flex justify-between text-gray-900 font-semibold mb-4">
                  <span>AED {Math.round(returns[4].rental).toLocaleString()}</span>
                  <span>AED {Math.round(returns[4].capital).toLocaleString()}</span>
                </div>
              </div>
                    ...
                    <div key={index} className="flex flex-col items-center">
                      <div className="flex flex-col items-center mb-2">
                        <div
                          ...
                          style={{ height: `${capitalHeight}%`, minHeight: "4px" }}
                        ></div>
                        <div
                          className="w-8 bg-green-500 rounded-b"
                          style={{ height: `${rentalHeight}%`, minHeight: "4px" }}
                        ></div>
                      </div>
                      <span className="text-xs text-gray-600 font-medium">{data.year} Year</span>
                    </div>
                  )
                })}
              </div>
              ...
              <div className="space-y-3 bg-gray-50 rounded-lg p-4">
                <div className="flex justify-between">
                  ...
                  <span className="text-gray-900 font-semibold">{avgReturn.toFixed(0)}%</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Total ROI</span>
                  <span className="text-gray-900 font-semibold">AED {Math.round(totalROI).toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Value of Investment After 5 Years</span>
                  <span className="text-accent font-bold">AED {Math.round(finalValue).toLocaleString()}</span>
                </div>
              </div>
            </div>
          </div>
